  ...
  task :countries => :environment do
    ...
    ActiveRecord::Base.transaction do
      MainSite::Region.find(:all, :select => 'id, name, name_ru, permalink').each do |region|       
        ...
        region_forum = Forum.create :name => _name, :site_id => site.id, :permalink => region.permalink if region_forum.nil?
        MainSite::Country.find(:all, :select => 'id, name, name_ru, permalink', :conditions => {:region_id => region.id}).each do |country|      
          ...
          Forum.create :name => _name, :site_id => site.id, :parent_id => region_forum.id, :permalink=> country.permalink if country_forum.nil?
        end
      end
    end
  end
